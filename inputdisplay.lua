...
function getControllerInput()
	...
	if input_array.A then
		press_a = 1;
	else
		press_a = 0;
	end
		
	if input_array.B then
		press_b = 1;
	else
		press_b = 0;
	end
	
	if input_array["C Down"] then
		press_cd = 1;
	else
		press_cd = 0;
	end
	
	if input_array["C Left"] then
		press_cl = 1;
	else
		press_cl = 0;
	end
		
	if input_array["C Right"] then
		press_cr = 1;
	else
		press_cr = 0;
	end
	
	if input_array["C Up"] then
		press_cu = 1;
	else
		press_cu = 0;
	end
		
	if input_array.Start then
		press_s = 1;
	else
		press_s = 0;
	end
		
	if input_array["DPad D"] then
		press_dd = 1;
	else
		press_dd = 0;
	end
		
	if input_array["DPad L"] then
		press_dl = 1;
	else
		press_dl = 0;
	end
		
	if input_array["DPad U"] then
		press_du = 1;
	else
		press_du = 0;
	end
		
	if input_array["DPad R"] then
		press_dr = 1;
	else
		press_dr = 0;
	end
		
	if input_array.L then
		press_l = 1;
	else
		press_l = 0;
	end
		
	if input_array.R then
		press_r = 1;
	else
		press_r = 0;
	end
		
	if input_array.Z then
		press_z = 1;
	else
		press_z = 0;
	end
		
	if input_array["X Axis"] ~= nil then
		ana_x = input_array["X Axis"];
	else
		ana_x = 0;
	end
	
	if input_array["Y Axis"] ~= nil then
		ana_y = input_array["Y Axis"];
	else
		ana_y = 0;
	end
end
